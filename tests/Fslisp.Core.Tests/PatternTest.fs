...
let patternBuildTest () =
    Assert.Equal(Ok { Fixed = []; Rest = None }, Pattern.build Sexp.Nil)
    Assert.Equal(Error(Sexp.Num 0.0), Pattern.build (Sexp.Num 0.0))
    Assert.Equal(Ok { Fixed = [ "a" ]; Rest = None }, Pattern.build (Sexp.List [ Sexp.Sym "a" ]))
    Assert.Equal(Ok { Fixed = [ "a"; "b" ]; Rest = None }, Pattern.build (Sexp.List [ Sexp.Sym "a"; Sexp.Sym "b" ]))
    Assert.Equal(Error(Sexp.Num 0.0), Pattern.build (Sexp.List [ Sexp.Sym "a"; Sexp.Num 0.0 ]))
    ...
    Assert.Equal(Ok { Fixed = []; Rest = Some "c" }, Pattern.build (Sexp.Sym "c"))
...
let patternBindTest () =
    ...
    let testcases =
        [ [], [ exact2, None; atLeast1, None ]
          [ Sexp.Num 0.0 ], [ exact2, None; atLeast1, Some [ "a", Sexp.Num 0.0; "b", Sexp.Nil ] ]
    ...
    for input, patternAndResults in testcases do
        for pattern, result in patternAndResults do
            ...
            | None, Error _ -> ()
            | Some r, Ok map -> Assert.Equal<Map<string, Sexp<unit>>>(Map.ofList r, map)
            | None, Ok _ -> Assert.True(false, sprintf "%A <- %A: Success" pattern input)
            | Some _, Error e -> Assert.True(false, sprintf "%A <- %A: %s" pattern input e)
