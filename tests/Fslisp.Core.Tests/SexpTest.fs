...
let sexpPatternConstructionTest () =
    Assert.Equal(Sexp.Nil, Sexp.List [])
    Assert.Equal(Sexp.Cons(Sexp.Bool true, Sexp.Nil), Sexp.List [ Sexp.Bool true ])
    ...
    Assert.Equal(Sexp.Cons(Sexp.Sym "quote", Sexp.Cons(Sexp.Bool true, Sexp.Nil)), Sexp.Quote(Sexp.Bool true))

    Assert.Equal(
        Sexp.Cons(
            Sexp.Sym "quasiquote",
            Sexp.Cons(Sexp.Cons(Sexp.Sym "unquote", Sexp.Cons(Sexp.Sym "foo", Sexp.Nil)), Sexp.Nil)
        ),
...
let sexpToString () =
    ...
    Assert.Equal(
        "'(#t #f c)",
        (Sexp.Quote(Sexp.List [ Sexp.Bool true; Sexp.Bool false; Sexp.Sym "c" ]))
            .ToString()
    )
