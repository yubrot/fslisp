...
let listSignatureTest () =
    let p = List(Sym "foo")
    Assert.Equal(Ok [ "a"; "b" ], tryMatch p (Sexp.List [ Sexp.Sym "a"; Sexp.Sym "b" ]))
...
let patSignatureTest () =
    let p = Pat "pattern"
    Assert.Equal(Ok { Fixed = [ "a"; "b" ]; Rest = None }, tryMatch p (Sexp.List [ Sexp.Sym "a"; Sexp.Sym "b" ]))
    Assert.Equal(Error(Sexp.Sym "pattern", InvalidPattern "123"), tryMatch p (Sexp.List [ Sexp.Num 123.0 ]))

[<Fact>]
let tupleSignatureTest () =
    Assert.Equal(Ok(), tryMatch () [])
    Assert.Equal(Error([], ArityMismatch), tryMatch () [ Sexp.Sym "a" ])
    Assert.Equal(Ok("test", ()), tryMatch (Sym "a", ()) [ Sexp.Sym "test" ])
    Assert.Equal(Error([ Sexp.Sym "a" ], ArityMismatch), tryMatch (Sym "a", ()) [ Sexp.Sym "test"; Sexp.Sym "over" ])
    Assert.Equal(Ok("test", "test2", ()), tryMatch (Sym "a", Sym "b", ()) [ Sexp.Sym "test"; Sexp.Sym "test2" ])
...
let placeholderSignatureTest () =
    Assert.Equal(Ok(Sexp.Sym "f", Sexp.Bool true, ()), tryMatch ("one", "two", ()) [ Sexp.Sym "f"; Sexp.Bool true ])

[<Fact>]
let restSignatureTest () =
    Assert.Equal(Ok(Sexp.Num 1.0, []), tryMatch ("head", Rest "tail") [ Sexp.Num 1.0 ])
