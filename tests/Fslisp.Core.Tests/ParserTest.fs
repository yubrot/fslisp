...
let parseAmbientTest () =
    Assert.Equal(Ok Sexp.Nil, parseToEnd sexp "test" "()")
    Assert.Equal(Ok Sexp.Nil, parseToEnd sexp "test" "   \t  \n  ()\n")
    Assert.Equal(Ok Sexp.Nil, parseToEnd sexp "test" "; comment\n(); comment")

[<Fact>]
let parseAtomTest () =
    Assert.Equal(Ok Sexp.Nil, parseToEnd sexp "test" "()")
    Assert.Equal(Ok(Sexp.Num 123.0), parseToEnd sexp "test" "123")
...
[<Fact>]
let parseListTest () =
    Assert.Equal(Ok(Sexp.List [ Sexp.Bool true ]), parseToEnd sexp "test" "(#t)")
    Assert.Equal(Ok(Sexp.List [ Sexp.Bool true; Sexp.Bool false; Sexp.Bool true ]), parseToEnd sexp "test" "(#t #f #t)")
