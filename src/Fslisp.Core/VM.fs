...
type VM(context: IContext, env: Env<Value>, code: Code<Value>) =
    ...
    member _.Push(value: Value) = stack <- value :: stack
    ...
    member _.Pop() : Value =
        ...
        | head :: rest ->
            stack <- rest
            head
        | _ -> raise (InternalErrorException "Inconsistent stack")
    ...
    member _.Leave() =
        ...
        | (lastEnv, lastCode) :: rest ->
            ...
            code <- lastCode
            dump <- rest
        | _ -> raise (InternalErrorException "Inconsistent dump")

    member self.Apply(f: Value, args: Value list) =
        match f with
        | Sexp.Pure(Native.Builtin builtin) -> builtin.Run self (f :: args)
        ...
        | _ -> raise (EvaluationErrorException "Cannot call: ")
    ...
    member self.RunInst(inst: Inst<Value>) =
        match inst with
        | Inst.Ldc constant -> self.Push constant
        | Inst.Ldv variable -> self.Push(env.Get variable)
        | Inst.Ldf(pattern, body) ->
            ...
            self.Push(Sexp.Pure(Native.Fun(clos)))
        | Inst.Ldm(pattern, body) ->
            ...
            self.Push(Sexp.Pure(Native.Macro(clos)))
        | Inst.Ldb name ->
            match context.Builtins.Get name with
            | Some builtin -> self.Push(Sexp.Pure(Native.Builtin builtin))
            | None -> raise (EvaluationErrorException("Unsupported builtin: " + name))
        | Inst.Sel(a, b) ->
