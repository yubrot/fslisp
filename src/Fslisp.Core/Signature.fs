...
type Pat = Pat of string with
    ...
    static member inline Instance(Placeholder, Pat label) =
        Sexp.Sym label
...
module private Tuple =
    ...
    let inline placeholder0 () =
        []
    let inline placeholder2 (a: ^a, b: ^b) =
    ...
    let inline placeholder5 (a: ^a, b: ^b, c: ^c, d: ^d, e: ^e) =
        placeholder a :: placeholder b :: placeholder c :: placeholder d :: placeholder e
