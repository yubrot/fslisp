...
module Command =
    ...
    let execute (ctx: Context) (command: Command) =
        match command with
        | ParseSuccess(input, output) -> parse input |> success output
        | ParseFailure input -> parse input |> failure
        | CompileSuccess(input, output) -> parse input >> ctx.Compile |> success output
        | CompileFailure input -> parse input >> ctx.Compile |> failure
        | EvalSuccess(input, output) -> parse input >> ctx.Eval |> success output
        | EvalFailure input -> parse input >> ctx.Eval |> failure
        | EvalAll input ->
...
module TestCase =
    let parse (src: Stream) : TestCase list =
        ...
        let readLines len =
            seq { for i in 1..len -> reader.ReadLine() } |> String.concat "\n"
        ...
        let readCommand header =
            ...
            | [| "EVAL_ALL"; input |] ->
                let input = readLines (int input)
                EvalAll input
            | _ -> failwithf "Unknown test command: %s" header
    ...
    let run (ctx: Context) { Header = header; Command = command } : bool =
        try
            ...
            false
        with CommandFailedException msg ->
