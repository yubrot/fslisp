    ...
    member self.Expand (recurse: bool) (expr: Value) : Value =
        ...
        | Sexp.List(m :: args) ->
            ...
            | Some(Sexp.Pure(Native.Macro closure)) ->
                ...
                | Error e -> raise (EvaluationErrorException("This macro " + e))
            | _ -> if recurse then self.ExpandChildren expr else expr
        | _ -> if recurse then self.ExpandChildren expr else expr

    member self.ExpandChildren(expr: Value) : Value =
        match expr with
        | Sexp.Cons(a, b) -> Sexp.Cons(self.Expand true a, self.ExpandChildren b)
        | expr -> expr
